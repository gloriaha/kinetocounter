...
def cluster_centers(center_list,not_tiny_list,num_clusters):
    ...
    for i in range(len(center_list)):
        ...
        if len(center_list[i][not_tiny_list[i]])>0:
            # perform kmeans
            kmeans_list.append(KMeans(n_clusters=num_clusters, random_state=0).fit(nm_list[not_tiny_list[i]]))
            # calculate how many centers are in each cluster
            kt_counts[i] = [np.sum(kmeans_list[i].labels_==j) for j in range(num_clusters)]
            print('time index', i)
...
def calc_point_vals(included_points, im):
    ...
    """
    included_vals = np.array([im[point[0]][point[1], point[2]] for point in included_points])
...
def fit_gauss_3d(idx, center_list, points_list, points_init_list, not_tiny_list, blurred_im_list):
    ...
    gauss_df.columns = ['x_1', 'y_1', 'z_1',
                         'background', 'A1',
                         'sig_x1', 'sig_y1', 'sig_z1']
    # calculate integrated intensities
    gauss_df['int_intensity'] = gauss_df['A1']*gauss_df['sig_x1']*gauss_df['sig_y1']*gauss_df['sig_z1']
    return gauss_df
